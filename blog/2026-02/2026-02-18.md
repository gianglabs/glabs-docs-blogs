...
title: "From Bash to Nextflow: Validated GATK Migration Using MD5 Checksums (Part 2)"
authors: [river]
...
process FASTQC {
    ...
    // See: https://riverxdata.com/blog/bioinformatics-containers-build-efficient-docker
...
workflow GATK_VARIANT_CALLING {
    ...
    main:
    // QC steps (can run in parallel for all samples)
    FASTQC(reads_ch)                           // Step 1: Parallel execution
    TRIM_GALORE(reads_ch)                      // Step 2: Parallel execution
    
    // Alignment (parallel for all samples, depends on trimming)
    BWA_MEM(TRIM_GALORE.out.reads)             // Step 3
    SAMTOOLS_SORT(BWA_MEM.out.bam)             // Step 4
    GATK_MARKDUPLICATES(SAMTOOLS_SORT.out.bam) // Step 5
    
    // BQSR (parallel for all samples)
    ...
    // Variant calling (parallel for all samples)
    GATK_HAPLOTYPECALLER(
        GATK_APPLYBQSR.out.bam,
        reference_ch
    )                                           // Step 9: GVCF mode
    
    // Genotyping (can batch multiple samples)
    GATK_GENOTYPEGVCFS(
        GATK_HAPLOTYPECALLER.out.gvcf.collect(),
        reference_ch
    )                                           // Step 10
    
    // Filtering (parallel branches for SNPs and Indels)
    GATK_SELECTVARIANTS_SNP(...)               // Step 11a
    GATK_SELECTVARIANTS_INDEL(...)             // Step 12a (runs in parallel with 11a)
    GATK_VARIANTFILTRATION_SNP(...)            // Step 11b
    GATK_VARIANTFILTRATION_INDEL(...)          // Step 12b
    
    // Merge and annotate
    GATK_MERGEVCFS(...)                        // Step 13
    SNPEFF_ANNOTATE(...)                       // Step 14
    
    emit:
    final_vcf = SNPEFF_ANNOTATE.out.vcf
}
```

**For 100 samples:**
- Steps 1-8 run in parallel (100 concurrent processes)
- Step 9 runs in parallel (100 concurrent HaplotypeCaller jobs)
- Step 10 batches all 100 GVCFs together (joint calling)
- Steps 11-16 run in parallel per sample

**Result:** 100 samples complete in ~2-3 hours instead of 27 hours!
...
- **Problem**: GATK CollectInsertSizeMetrics requires R to generate PDF histogram, but standard biocontainers don't include R
- **Solution**: Switched from `quay.io/biocontainers/gatk4` to `broadinstitute/gatk:4.4.0.0` which includes R runtime
...
### 11.1. Migration is Systematic, Not Guesswork
...
### 11.2. The `.command.sh` is What Matters
...
### 11.3. Not All Differences Are Bugs
...
## 12. Bash vs Nextflow: When to Use Each
...
### 13.3. Next Steps
- **Part 3**: Complete 16-step Nextflow pipeline with all GATK processes (coming soon)
- **Part 4**: Advanced Nextflow features - error handling, retries, and resource optimization
- **Part 5**: Scaling to HPC clusters (Slurm) and cloud platforms (AWS Batch)

---

## 14. Conclusion

Migrating from bash to Nextflow isn't about blind rewriting—it's about **validated transformation**. We successfully migrated **all 13 core GATK steps** from bash to Nextflow with Singularity containers, completing the pipeline in just 2 minutes with 35 output files. While MD5 checksums differ due to timestamps and execution environment metadata, the scientific content (quality metrics, alignments, variant calls) is equivalent.

**Key achievements in this migration:**
- ✅ **Complete pipeline**: 13 steps migrated (FastQC → variant calling → filtering → merging)
...
- ✅ **Fast execution**: 2m 5s for single sample with automatic caching and resume capability
...
### 14.1. What's Next: Part 3 - Production-Grade Deployment

In **Part 3**, we'll take this pipeline to **production scale** with:

**🎯 SLURM Integration:**
- Configure Nextflow executor for HPC clusters
- Resource allocation strategies (CPU, memory, walltime)
- Job array parallelization for 100+ samples
- Queue management and scheduling optimization

**📊 Real-World Validation:**
- Use authentic human genome sequencing data (not toy datasets)
- Test with multiple samples from **1000 Genomes Project (1KGP)**
- Compare variant calling results against 1KGP gold standard
- Benchmark runtime: bash (serial) vs Nextflow (parallel)

**🔬 Quality Verification:**
- Concordance analysis: our calls vs. 1KGP reference panel
- Sensitivity/specificity metrics for SNPs and indels
- Coverage distribution and mapping statistics
- Mendelian inheritance error rates (trio samples)

**🚀 Performance at Scale:**
- Single sample: 2 minutes (this blog)
- 100 samples: ~2-3 hours (Part 3 target)
- 1000 samples: ~1-2 days (vs. weeks in bash)

**📦 Optional Enhancements:**
- SnpEff functional annotation
- bcftools variant statistics and filtering
- MultiQC aggregate reporting
- Samplesheet-driven CSV input for batch processing

**Ready to scale your pipeline?** Part 3 will show you how to go from proof-of-concept to production-grade infrastructure!

---

### 14.2. Ready to Migrate Your Own Pipeline?
